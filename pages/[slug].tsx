...
const RoutePage = ({
  ...
  return (
    <motion.div
      className={!isSmall ? 'relative overflow-hidden' : 'min-h-screen bg-white'}
      style={!isSmall ? { height: '100dvh' } : undefined}
      initial={{ x: 430 }}
      animate={{ x: 0 }}
    ...
    >
      ...
      {route && (
        <>
          ...
          {/* Nav — absolute on mobile, sticky on desktop */}
          <nav
            className={`${
              !isSmall ? 'absolute top-0 left-0 right-0 z-30' : 'sticky top-0 z-10'
            } flex items-center justify-between px-5 py-3 -mx-5 border-b border-gray-200 bg-blur`}
          >
            <Link href="/">
              <a>
                <svg
          ...
          {/* Mobile: bottom sheet with all content */}
          {!isSmall ? (
            <BottomSheet collapsedHeight={260}>
              <header className="text-center py-4">
                <h1 className="text-2xl font-bold text-forest-darkest tracking-tight mb-1">{name}</h1>
                {(route.location || route.swimrun) && (
                  <div className="flex items-center justify-center gap-2">
                    {route.location && (
                      <span className="text-xs font-semibold tracking-wide uppercase text-gray-400">{route.location}</span>
                    )}
                    {route.swimrun && <span className="text-xs font-semibold tracking-wide text-blue-500 uppercase">Swimrun</span>}
                  </div>
                )}
              </header>
              <ul className="grid grid-cols-4 gap-2 mb-4">
                <Stat type="Distanz" value={`${Math.round(route.distance * 10) / 10} km`} centered className={statBoxClassName} />
                <Stat type="Aufstieg" value={`${Math.round(route.elevation)} m`} centered className={statBoxClassName} />
              ...
              <div className="p-3 mb-4 border border-gray-200 rounded-xl">
                <Chart coordinates={route.gpxGeoJson.features[0].geometry.coordinates} type={route.gpxGeoJson.features[0].geometry.type} />
              </div>
              {route.description && <p className="mb-3 leading-relaxed whitespace-pre-wrap text-sm">{route.description}</p>}
              {link && (
                <p className="mb-5 text-sm">
                  Auf{' '}
                  <a className="text-forest" href={link} target="_blank" rel="noopener noreferrer">
                    Strava
                  </a>{' '}
                  ansehen.
                </p>
            ...
            <>
              ...
              <div className="p-3 mb-3 border border-gray-200 rounded-xl">
                <Chart coordinates={route.gpxGeoJson.features[0].geometry.coordinates} type={route.gpxGeoJson.features[0].geometry.type} />
              </div>
